...
static inline char *remove_leading_zero(char *num)
{
...
static inline void itoa(svalue_t * sp)
{
...
*/
/* n1, n2 : 正整數 */
static inline int compare_value(int negative1, char *n1, int negative2,
				char *n2, const char *op)
{
...
/* 加法處理函式 */
/* n1, n2 : 正整數 */
static inline char *addition(char *n1, char *n2)
{
	...
	for (i = n1_len - 1, j = n2_len - 1, k = ans_len - 1; i >= 0 && j >= 0;
	     i--, j--, k--) {
...
/* 減法處理函式 */
/* n1 : 大正整數, n2 : 小正整數 */
static inline char *subtraction(char *n1, char *n2)
{
	...
	for (i = n1_len - 1, j = n2_len - 1, k = ans_len - 1; i >= 0 && j >= 0;
	     i--, j--, k--) {
		ans[k] =
		    (n1[i] - 48) - (n2[j] - 48) + carry + ((n1[i] + carry) <
							   n2[j] ? 10 : 0) + 48;
	...
	if (i >= 0)
		for (; i >= 0; i--, k--) {
			ans[k] =
			    n1[i] + carry + ((n1[i] - 48 + carry) < 0 ? 10 : 0);
...
/* 乘法處理函式 */
/* n1, n2 : 正整數 */
static inline char *multiplication(char *n1, char *n2)
{
...
static inline char *division(char *n1, char *n2)
{
	...
	for (i = 0; i < n1_len; i++, carry = 0) {
		if (!i) {
			...
			tmp_len = strlen(tmp);
			str =
			    (char *)DMALLOC(tmp_len + 2, TAG_STRING,
					    "division 3");
			strcpy(str, tmp);
			str[tmp_len] = n1[i];
			str[tmp_len + 1] = '\0';
		...
		while (compare_value(0, tmp, 0, n2, ">=")) {
			...
			FREE(tmp);
			tmp =
			    (char *)DMALLOC(strlen(str) + 1, TAG_STRING,
					    "division 4");
			strcpy(tmp, str);
			FREE(str);
			carry++;
...
void f_count()
{
	...
	if ((sp - 2)->u.string[0] == 45) {
		negative1 = 1;
		tmp =
		    (char *)DMALLOC(strlen((sp - 2)->u.string), TAG_STRING,
				    "count");
		(sp - 2)->u.string++;
		...
		tmp =
		    (char *)DMALLOC(strlen((sp - 2)->u.string) + 1, TAG_STRING,
				    "count");
		strcpy(tmp, (sp - 2)->u.string);
	}
	...
	if (sp->u.string[0] == 45) {
		negative2 = 1;
		tmp =
		    (char *)DMALLOC(strlen(sp->u.string), TAG_STRING, "count");
		sp->u.string++;
		strcpy(tmp, sp->u.string);
		sp->u.string--;
	} else {
		tmp =
		    (char *)DMALLOC(strlen(sp->u.string) + 1, TAG_STRING,
				    "count");
		strcpy(tmp, sp->u.string);
	}
	...
	case 1:
		{
			...
			case '*':
				if (((negative1 && !negative2)
				     || (negative2 && !negative1))
				    && !compare_value(0, n1, 0, "0", "==")
				    && !compare_value(0, n2, 0, "0", "=="))
					negative = 1;
			...
			case '/':
				...
				break;
			default:
				i = compare_value(negative1, n1, negative2, n2,
						  op);
