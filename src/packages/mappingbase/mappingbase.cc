...
void f_mb_set(void)
{
	...
	case 4:
		if (sp->type == T_OBJECT)
			...
			POP_ERR
			    ("Bad argment 4 to efun mb_set()\nExpect : Object\n")
		    if ((sp - 1)->type == T_STRING)
			tmpstr = (char *)(sp - 1)->u.string;
		else
			POP_ERR
			    ("Bad argment 3 to efun mb_set()\nExpect : String\n")
			    break;
	case 3:
...
void f_mb_query(void)
{
	...
	case 3:
		if (sp->type == T_OBJECT)
			...
			POP_ERR
			    ("Bad argment 3 to efun mb_query()\nExpect : Object \n");
		if ((sp - 1)->type == T_STRING)
			tmpstr = (char *)(sp - 1)->u.string;
		else
			POP_ERR
			    ("Bad argment 2 to efun mb_query()\nExpect : String\n")
			    break;
	case 2:
		if (sp->type == T_MAPPING) {
			map = sp->u.map;
...
void f_mb_delete(void)
{
	...
	case 3:
		if (sp->type == T_OBJECT)
			...
			POP_ERR
			    ("Bad argment 3 to efun mb_delete()\nExpect : Object \n")
		    if ((sp - 1)->type == T_STRING)
			tmpstr = (char *)(sp - 1)->u.string;
		else
			POP_ERR
			    ("Bad argment 2 to efun mb_delete()\nExpect : String\n")
			    break;
	case 2:
		if (sp->type == T_MAPPING) {
			map = sp->u.map;
