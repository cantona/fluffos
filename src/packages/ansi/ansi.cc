...
const char *code[] =
    { "$NOR$", "$BLK$", "$RED$", "$GRN$", "$YEL$", "$BLU$", "$MAG$", "$CYN$",
	"$WHT$",
	"$HIR$", "$HIG$", "$HIY$", "$HIB$", "$HIM$", "$HIC$", "$HIW$",
	"$HBBLK$", "$HBRED$", "$HBGRN$", "$HBYEL$", "$HBBLU$", "$HBMAG$",
	"$HBCYN$", "$HBWHT$",
...
const char *color[] =
    { "[m", "[30m", "[31m", "[32m", "[33m", "[34m", "[35m", "[36m",
	"[1;30m",
	"[1;31m", "[1;32m", "[1;33m", "[1;34m", "[1;35m", "[1;36m",
	    "[1;37m",
	"[40;1m", "[41;1m", "[42;1m", "[43;1m", "[44;1m", "[45;1m",
	    "[46;1m", "[47;1m",
	"[40m", "[41m", "[42m", "[43m", "[44m", "[45m", "[46m", "[47m",
	"[1m", "[1;7m"
};

static inline int combine_ansi(char **ret, char *str, char **ansi_table,
			       int *flag);
static inline char *rally_ansi(char *str);

//
// 濾除 \0 至 \x20 之間的控制字元
// 並依照上表將特定字串轉換成 ANSI 色碼
//
#ifdef F_ANSI
void f_ansi()
{
...
//
// 濾除重覆且不必要的 ANSI 色碼並將多組色碼組合起來以減低網路頻寬負載
//
...
static inline char *rally_ansi(char *str)
{
	...
	for (j = 0; j < ANSI_CAP; j++) {
		...
		*new_table[j] = *(new_table[j] + 1) = *last_table[j] =
		    *(last_table[j] + 1) = *ansi_table[j] =
		    *(ansi_table[j] + 1) = 0;
	}
	...
	while (*str) {
		if (*str != '\x1B') {
			*ret++ = *str++;
			...
			str +=
			    combine_ansi((char **)&ret, str, ansi_table, &flag);
