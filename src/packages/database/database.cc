...
static inline svalue_t *insert_in_mapping(mapping_t * m, char *key)
{
	svalue_t lv;
	svalue_t *ret;
...
void f_set()
{
	...
	dst = tmpstr =
	    (char *)DMALLOC(SVALUE_STRLEN(sp - 1) + 1, TAG_STRING, "set");
	j = 0;
...
void f_set_temp()
{
	...
	src = (char *)(sp - 1)->u.string;
	dst = tmpstr =
	    (char *)DMALLOC(SVALUE_STRLEN(sp - 1) + 1, TAG_STRING, "set_temp");
	j = 0;
...
void f_query()
{
	...
	src = (char *)sp->u.string;
	dst = tmpstr =
	    (char *)DMALLOC(SVALUE_STRLEN(sp) + 1, TAG_STRING, "query");
	...
	if (value == &const0u) {
		...
		if (shadow != &const0u && shadow->type == T_OBJECT
		    && ob != shadow->u.ob) {
			idx =
			    find_global_variable(shadow->u.ob->prog, "dbase",
						 &type, 0);
...
void f_query_temp()
{
	...
	src = (char *)sp->u.string;
	dst = tmpstr =
	    (char *)DMALLOC(SVALUE_STRLEN(sp) + 1, TAG_STRING, "query_temp");
...
void f_addn()
{
	...
	src = (char *)(sp - 1)->u.string;
	dst = tmpstr =
	    (char *)DMALLOC(SVALUE_STRLEN(sp - 1) + 1, TAG_STRING, "addn");
	j = 0;
...
void f_addn_temp()
{
	...
	src = (char *)(sp - 1)->u.string;
	dst = tmpstr =
	    (char *)DMALLOC(SVALUE_STRLEN(sp - 1) + 1, TAG_STRING, "addn_temp");
	j = 0;
